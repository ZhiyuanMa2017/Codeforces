...
//        }
    ...
    public static void solve(Kattio kattio) {
        ...
        for (int i = 1; i <= n; i++) {
            next[i] = kattio.nextInt();
            inDegree[next[i]]++;
        }
        for (int i = 1; i <= n; i++) {
            if (next[i] == 0 && inDegree[i] == 0) {
                zeros.add(i);
            } else if (next[i] != 0 && inDegree[i] == 0) {
                head.add(i);
            }
        }
        for (int i = 0; i < zeros.size() - 1; i++) {
            next[zeros.get(i)] = zeros.get(i + 1);
        }
        if (zeros.size() > 0) {
            head.add(zeros.get(0));
        }
        for (int i = 0; i < head.size(); i++) {
            int curHead = head.get(i);
            int nextHead = head.get((i + 1) % head.size());
            while (next[curHead] != 0) {
                curHead = next[curHead];
            }
            next[curHead] = nextHead;
        }
