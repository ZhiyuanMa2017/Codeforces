...
//        solve(kattio);
    ...
    public static void solve(Kattio kattio) {
        ...
        Deque<Integer> lastZero = new ArrayDeque<>();
        for (int i = 0; i < n; i++) {
            int num = s.charAt(i) - '0';
            if (num == 0) {
                if (!lastOne.isEmpty()) {
                    int seqNum = lastOne.pollFirst();
                    res.append(seqNum).append(" ");
                    lastZero.addLast(seqNum);
                } else {
                    res.append(cur).append(" ");
                    lastZero.addLast(cur);
                    cur++;
                }
            } else {
                if (!lastZero.isEmpty()) {
                    int seqNum = lastZero.pollFirst();
                    res.append(seqNum).append(" ");
                    lastOne.addLast(seqNum);
                } else {
                    res.append(cur).append(" ");
                    lastOne.addLast(cur);
                    cur++;
